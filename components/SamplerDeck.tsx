    ...
    const loadSample = async (url: string, name: string) => {
        ...
        const player = new window.Tone.Player({
            ...
            onload: () => {
                ...
                if (filterRef.current) player.connect(filterRef.current);
    ...
    return (
        <div className="flex flex-col h-full w-full gap-2 overflow-hidden">
            ...
            <div className="flex-1 bg-slate-900 rounded-xl border-t border-slate-700 shadow-2xl p-1 md:p-2 grid grid-cols-12 gap-2 min-h-0 overflow-hidden relative">
                 ...
                 {/* LEFT: DRUM PADS */}
                 <div className={`col-span-12 lg:col-span-3 flex-col gap-2 z-10 min-h-0 h-full
                 ...
                 {/* CENTER: STUDIO/LIBRARY */}
                 <div className={`col-span-12 lg:col-span-6 flex-col gap-2 z-10 min-h-0 h-full overflow-hidden
                    ${activeTab === 'studio' ? 'flex' : 'hidden lg:flex'}`}>
                     
                     {/* LCD Screen - Dynamic Height! Shrinks on small landscape screens */}
            ...
            {/* Fixed height for reliability on mobile landscape, dynamic on desktop */}
